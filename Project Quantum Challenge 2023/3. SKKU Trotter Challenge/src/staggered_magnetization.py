    ...
    def energies(self, states):
        ...
        for i, state in enumerate(states):
            for qubit in range(n):
                # Map the binary state to the eigenvalues of the Z operator
                # (0 -> +1, 1 -> -1) and apply staggered sign
                spin_state = 1 if state[qubit] == 0 else -1
                energies[i] += (1 / n) * ((-1) ** qubit) * spin_state
        """
        energies = np.zeros(len(states))

        # Iterate over each state
        energies = np.array([
            sum((-1) ** j * self.hamiltonian.coeffs[j] * (1 if state[j] == '0' else -1) for j in range(len(state)))
            for state in states
        ])
